...
const GlobalsSection = ({ editing, setSection, sectionInformation, removeSub }: Props) => {
    ...
    (node: any) => {
      if (node !== null) {
        ...
        for (const [k, error] of Object.entries(node.errors)) {
          invalidValues.push({ key: `globals.${k}`, error: error as string | object });
        }
  ...
  return (
    <Formik
      key={formKey}
      innerRef={sectionRef}
      initialValues={{ ...sectionInformation.data, name: 'Globals' }}
